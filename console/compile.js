...
(function () {
    ...
    switch (contentType) {
      case "object":
        ...
        if (visited.includes(data)) {
          str +=
            '<span class="string">\u68C0\u6D4B\u5230\u5FAA\u73AF\u5F15\u7528</span>';
        } else {
          visited.push(data);
          ...
          if (len <= 0) {
            ...
            str += hasKey
              ? '<span class="bracket">{ }'.concat(lastComma, "</span>")
              : '<div class="bracket">{ }'.concat(lastComma, "</div>");
          } else {
            // 非空对象
            // expandBtn是展开和收缩按钮
            ...
            str += hasKey
              ...
              : '<div class="bracket">{</div>'; // 这个wrap的div用来实现展开和收缩
            ...
            keys.forEach(function (key, index) {
              ...
              str +=
                '\n                                <div class="object">\n                                    <span class="key">"'
                  ...
                  .concat(stringify(data[key], true, index >= len - 1, visited))
                  .concat(
        ...
        if (visited.includes(data)) {
          ...
          if (len <= 0) {
            ...
            data.forEach(function (item, index) {
              ...
              str +=
                '\n                            <div class="array">\n                                '
                  .concat(stringify(item, true, index >= len - 1, visited))
                  .concat(
                    index < len - 1 ? "," : "",
                    "\n                            </div>"
                  );
      ...
      default:
        str += '<span class="'
          .concat(res.contentType, '">')
          .concat(quotationMarks)
          .concat(res.content)
          .concat(quotationMarks, "</span>");
        break;
    }
  ...
  var handleData = function handleData(content) {
    ...
    switch (contentType) {
      ...
      case "object":
        // 对象
        content = stringify(JSON.stringify(content), false, true, [], true);
        break;
        ...
        if (match) {
          ...
          match.forEach(function (item, index) {
            ...
            switch (placeholder) {
              ...
              case "%i":
                newStr =
                  (type(arg) === "number" ? parseInt(arg) : "NaN") +
                  item.slice(2);
                break;
              // 浮点数
    ...
    switch (method) {
      ...
      case "timeEnd":
        if (timeData[contents[0]]) {
          contents = [
            contents[0] + ": " + (Date.now() - timeData[contents[0]]) + " ms",
          ];
        } else {
          contents = null;
        }
