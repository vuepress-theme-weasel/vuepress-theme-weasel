...
const getPlugins = (pkgPath) => {
  return [
    vue(),
    typescript({
      tsconfig: path.resolve(pkgPath, './tsconfig.build.json')
    }),
    nodeResolve({
...
module.exports = fs
  ...
  // 为每一个文件夹创建对应的配置
  .map(item => {
    const resolvePkgPath = path.resolve(root, item)
    const pkg = require(path.resolve(root, item, 'package.json'))
    const inputFile = path.resolve(root, item, 'src/node/index.ts')
    return {
      input: inputFile,
      output: [
        {
          file: path.resolve(resolvePkgPath, pkg.main),
          format: 'cjs',
          exports: "named",
          sourcemap: isDev
        }
      ...
      plugins: getPlugins(resolvePkgPath),
