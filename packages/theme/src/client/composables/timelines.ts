...
import type { Article } from "@mr-huang/vuepress-plugin-blog";
...
export const setupTimelines = (): void => {
  const timelines = useBlogPageType<ArticleInfo>("timeline");
  ...
  const timelineItems = computed(() => {
    ...
    timelines.value.items.forEach(({ info, path }) => {
      // @ts-ignore
      const { year, month, day } = getDate(info.date, { type: "date" })?.detail || {};
