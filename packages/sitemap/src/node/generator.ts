...
import type { ModifyTimeGetter, SitemapFrontmatterOption, SitemapImageOption, SitemapLinkOption, SitemapNewsOption, SitemapOptions, SitemapVideoOption } from './../typings';
 ...
 * @returns 
 */
const generatePageMap = (app: App, options: SitemapOptions): Map<string, SitemapPageInfo> => {
  const {
    changefreq,
 ...
 * @param options 
 */
export const createSitemap = async (app: App, options: SitemapOptions) => {
  const { extraUrls = [], xmlns } = options;
